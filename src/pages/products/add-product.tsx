...
import { useCreateProductMutation } from "../../store/services/productsApiSlice";
...
const AddProducts = () => {
    ...
    const onSubmit: SubmitHandler<Inputs> = (data) => {
        data.images.pop();
        UseCustomToast(createProduct(data), "Adding Product");
    }
    const categories = dataResolver.resolveCatalogAutoComplete(allCategories?.data || []);

    return (
        <Box>
            <Typography variant="h6" fontWeight={600}>Add Product</Typography>
            <form onSubmit={handleSubmit(onSubmit)}>
                <Box sx={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                    ...
                    <Box>
                        <Button variant="contained" color="primary" fullWidth type="submit">Add Product</Button>
                    </Box>
                </Box>
            </form>
        </Box>
