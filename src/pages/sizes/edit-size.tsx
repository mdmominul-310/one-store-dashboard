...
import { useGetSingleSizeQuery, useUpdateSizeMutation } from "../../store/services/sizeApiSlice";
type Category = {
    name: string;
    description: string;
}
const EditSize = () => {
    ...
    const { data, isLoading, isError } = useGetSingleSizeQuery(catid);
    // eslint-disable-next-line no-unsafe-optional-chaining
    const [updateSize] = useUpdateSizeMutation();
    const onSubmit: SubmitHandler<Category> = (data: Category) => {
        UseCustomToast(updateSize({ ...data, id: catid }), 'updating Size');
    }
    ...
    return (
        <Container>
            <Typography variant="h6" fontWeight={600}>Edit Size</Typography>
            {
